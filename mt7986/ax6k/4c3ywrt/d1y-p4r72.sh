...
sed -i '/rcd -vv/a \\tprocd_append_param command "--rc-web-gui"\n\tprocd_append_param command "--rc-files=/www/rclone-webui-react"' feeds/packages/net/rclone/files/rclone.init
...
# upgrade config
wget --no-check-certificate -O package/base-files/files/etc/uci-defaults/zzz-updata-settings "https://raw.githubusercontent.com/0xACE8/m3d14tek/main/mt7986/ax6k/4c3ywrt/zzz-updata-settings"
